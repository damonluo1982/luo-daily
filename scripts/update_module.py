...
①今日收盘速览（指数卡片：上证/深成/创业板/沪深300/科创50的收盘点位+涨跌幅，两市成交额、北向资金、涨跌比，后接1段盘面要点；盘面要点之后紧接"上证指数·近5日"一行紧凑数据：**必须逐日直接使用下方提供的"上证指数API数据"，禁止编造、禁止留空**，flex横向排列每日数据：日期+收盘点位+涨跌幅，当日数据可加粗）
②今日要闻（紧随盘面要点之后，3-5条超短要闻每条一行，只放与A股投资直接相关的当日要闻：政策发布/宏观数据/行业大事/海外市场影响等；**首条固定为隔夜美股行情一句话：道指/纳指/标普500收盘点位+涨跌幅，必须直接使用下方提供的"美股API数据"，禁止编造、禁止留空**）
...
⑦本周关键事件（3-5条）
⑧黄金·过去一周每日收盘（单位人民币元/克。**必须逐日直接使用下方提供的"沪金API数据"，禁止编造、禁止留空**；API给几个交易日就展示几个，涨跌幅也用API数据；若API数据缺失才可退回搜索SGE Au9999并标注"按搜索数据"。flex横向排列每日数据：日期+收盘价（元/克）+涨跌幅，底部1行周涨幅摘要）
...
末尾加 <div class="disc">⚠️ 数据来自公开渠道，法拍房有风险，投资需谨慎。</div>""",
    ...
    "salary": {
        ...
        "queries": [
            ...
            "美世 Mercer 怡安 Aon WTW 韬睿惠悦 2026 薪酬调研 涨薪 人才趋势 报告",
            "光辉国际 Korn Ferry 合益 Hay 罗盛 Russell Reynolds 薪酬 人才趋势 报告",
            "德勤 Deloitte 人力资本 薪酬报告 中国 涨薪预测",
            "深圳 最低工资 社保缴费基数 最新 政策",
            "AI人才 芯片 半导体 薪酬 最新",
            "人社部 工资收入分配 政策 最新 发布",
            "全国 各省 最低工资标准 调整 最新 通知",
        ],
        "prompt": """请更新「薪酬COE热点」模块，按以下框架：
①核心速览（3-5条关键洞察卡片，前置）
②外部动态（紧随核心速览，3-6条每条一行超短要闻，形式参照A股"今日要闻"的紧凑样式；每条前加来源标签，[国家]类：人社部/国家层面工资收入分配政策、各省最低工资调整、社保个税变化、国企工资总额管理等；[同行]类：腾讯/字节/比亚迪/小米/中兴/荣耀等关键人才的调薪、激励、组织薪酬大动作；此处只做超短速览，详细分析放后续板块展开，避免内容重复）
③华为竞争对手薪酬动态（腾讯/字节/比亚迪/小米/中兴/荣耀等：调薪、股权激励、应届生薪酬包）
④咨询公司人才趋势（不限于美世Mercer/怡安Aon/WTW韬睿惠悦，其他有影响力的咨询公司报告同样收录：光辉国际Korn Ferry/合益Hay/罗盛Russell Reynolds/史宾沙Spencer Stuart/德勤Deloitte人力资本/博思艾伦等。内容：年度薪酬调研/涨薪预测/AI对人才影响/行业分化/高管薪酬趋势。每条注明来源公司，多家观点可对比呈现，有分歧就并列展示）
...
末尾加 <div class="disc">⚠️ 信息来自公开渠道，仅供内部参考。</div>""",
    ...
    "huawei": {
        "name": "华为最新消息",
        "queries": [
            "华为 2026 最新 业绩 营收 任正非 讲话",
            "华为 2026 手机 出货量 市场份额 最新",
            "鸿蒙智行 问界 2026 交付 最新",
            "华为 昇腾 麒麟 芯片 2026 最新 进展",
            "华为 HarmonyOS 鸿蒙 2026 生态 装机量 最新",
            "华为 2026 海外市场 最新动态",
            "苹果 三星 小米 荣耀 中兴 2026 手机 出货量 销量",
            "理想 蔚来 小米汽车 比亚迪 2026 新势力 交付",
            "英伟达 高通 联发科 2026 芯片 最新 动态",
        ],
        "prompt": """请更新「华为最新消息」模块，按以下框架：
【时效性硬规则（最高优先级，违反即重做）】所有信息必须是近1年内的（优先近1个月）；每条内容尽量标注发生日期；发布超过1年的信息、无法确认时效的信息一律不得采用；任正非讲话/组织变动必须是近1年内的；财务数据必须用最新一期财报，禁止使用超过1个报告期的旧营收数据（旧财报只能作为背景补充且必须标注年份）；本模块是"最新消息"，旧闻没有意义，宁缺毋滥。
①核心速览（3-5条最重要的动态卡片）
②同行动态（紧随核心速览，3-6条每条一行超短要闻，形式参照A股"今日要闻"的紧凑样式；每条前加领域标签：[手机]类=苹果/三星/小米/荣耀/中兴等终端销量与新品动作，[汽车]类=理想/蔚来/小米汽车/比亚迪等智行竞品的交付、销量、新车动作，[芯片/云]类=英伟达/高通/联发科等芯片与算力动态；优先选可与华为对标的标志性事件，此处只做超短速览，详细对比放后续板块，避免内容重复）
③集团动态（财务卡片：最新一期营收/利润/研发投入，任正非近期讲话，组织变动）
④各产业板块经营（终端/ICT基础设施/企业业务云计算/智能汽车/数字能源/海思）
⑤全球市场份额（手机/5G设备/云计算/智能汽车交付排名，可与同行对比呈现）
...
def build_data_block(module):
    ...
    if module == "stock":
        ...
        if s:
            parts.append(s + "\n")
        g = fetch_stock.fetch_gold()
        if g:
            parts.append("【沪金API数据（新浪财经，可信度高，黄金板块必须以此为准）】\n" + g + "\n")
        s5 = fetch_stock.fetch_sh_index_5d()
        if s5:
            parts.append("【上证指数API数据（腾讯财经，可信度高，'上证指数·近5日'必须以此为准）】\n" + s5 + "\n")
        us = fetch_stock.fetch_us_index()
        if us:
            parts.append("【美股API数据（腾讯财经，可信度高，'今日要闻'首条美股行情必须以此为准）】\n" + us + "\n")

    parts.append("【网络搜索结果（按相关性，可能有噪声）】")
    parts.append("【时效性硬规则：每条结果前的[日期]是该条的发布日期；发布日期超过1年的结果已过滤掉；没有日期前缀的结果必须谨慎甄别，无法确认是近1年内信息的一律不采用；禁止使用你训练知识中的旧新闻/旧数据，只能用下方搜索结果和API数据。今天是%s，超过1年的信息对本模块毫无意义。】" % today_str())
    for i, query in enumerate(cfg["queries"]):
        results = tavily_search(query, max_age_days=365)
        parts.append("\n搜索%d: %s" % (i + 1, query))
        if not results:
            parts.append("（无结果）")
        for r in results:
            date_tag = ("[%s] " % r["published_date"]) if r.get("published_date") else "[日期不详] "
            parts.append("- %s[%s] %s" % (date_tag, r["title"][:60], r["content"][:500]))
    return "\n".join(parts)
